...
analyseInputJson = (file) => {
    ...
    reader.addEventListener('load', async (event) => {
        ...
        numberOfTotalKeysObj = await makeRequest("POST", "/saveDataFromFile/" + languageSelection.code, myDataRaw);
...
saveInFirestore= () => {
    ...
    if(Object.keys(correctMachineTranslations).length > 0){
        makeRequest("POST", "/saveInFirestore/" + languageSelection.code, JSON.stringify(correctMachineTranslations));
    }
}
